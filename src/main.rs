...
use std::{process::exit, string::String, path::Path};
...
fn writeOutput(fileName: String, outputData: String){
	let file = OpenOptions::new().write(true).append(true).create(true).open(fileName).expect("Unable to open file!");
	let mut file = BufWriter::new(file);
	file.write_all(outputData.as_bytes()).expect("Unable to write data to file!");
}
				...
				match reqwest::Client::builder().danger_accept_invalid_certs(true).build().unwrap().get(&url).send().await {
					Ok(resp) => {
						...
						match resp.text().await {
								...
								match platformName == "None" {
									true => {
										if args.is_present("verbose"){
											println!("[{}] {}!", Colour::Blue.bold().paint("Not Vulnerable"), url);
										}
									},
									_ => {
										println!("[{}]\t{} at {}!", Colour::Red.bold().paint(&platformName), Colour::White.bold().paint("Possible Sub-domain Takeover"), url);
										if args.is_present("output"){
							...
							Err(_) => {
								if args.is_present("verbose"){
									println!("[{}]\tAn error occured for [{}].", Colour::Green.bold().paint("ERROR"), Colour::White.bold().paint(url))
								}
							},
						}
					}
